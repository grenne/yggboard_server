...
public class AreaAtuacao {
	...
	public BasicDBObject get(String id, MongoClient mongo) {
		...
		BasicDBObject result = commons_db.getCollection(id, "areaAtuacao", "_id", mongo, false);
	...
	public JSONArray getAll(String usuarioParametro, MongoClient mongo) {
		...
		JSONArray array = commons_db.getCollectionListaNoKey("areaAtuacao", mongo, false);
		for (int i = 0; i < array.size(); i++) {
			...
			if (userPerfil != null) {
				if (userPerfil.get("areaAtuacao") != null) {
  				ArrayList<String> itens = (ArrayList<String>) userPerfil.get("areaAtuacao");
  				item.put("possui", commons.testaElementoArray(item.getString("id"), itens));
				};
				if (userPerfil.get("areaAtuacaoInteresse") != null) {
  				ArrayList<String> itens = (ArrayList<String>) userPerfil.get("areaAtuacaoInteresse");
	...
	public JSONArray getIdNome(String usuarioParametro, MongoClient mongo) {
		...
		JSONArray resultAll = commons_db.getCollectionListaNoKey("areaAtuacao", mongo, false);
